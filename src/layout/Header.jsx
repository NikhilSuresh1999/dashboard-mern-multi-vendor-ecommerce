...
const Header = ({ showSidebar, setShowSidebar}) => {
    ...
    <div className='fixed top-0 left-0 w-full py-5 px-2 lg:px-7 z-40'>
      <div className='ml-0 lg:ml-[260px] rounded-md h-[65px] flex justify-between items-center bg-[#b1addf] px-5 transition-all'>
        ...
        <div className='hidden md:block'>
          <input className='px-3 py-2 outline-none border bg-transparent border-slate-700 rounded-md text-[#575194] focus:border-indigo-300 overflow-hidden' type='text' name='search' placeholder='search'/>
        </div>
  ...
  userInfo?.role === 'admin' ? (
    ...
    <img
      className='w-[45px] h-[45px] rounded-full overflow-hidden'
      src={userInfo?.image?.url || "http://localhost:3000/images/demo.jpg"}
      alt=""
    />
  )
}
