...
const Profile = () => {
  ...
  //const image = true
  //const loader = true ----static data for design
  const status = 'active'
...
const handleDeleteStripe = async () => {
  const confirmDelete = window.confirm("Remove Stripe account?");

  if (!confirmDelete) return;

  try {
    await axios.delete('http://localhost:5000/api/delete-stripe-account', {
  withCredentials: true
});
    toast.success("Stripe account removed");
...
//////// Change Password 
      ...
      <div className='w-full flex flex-wrap'>
        ...
        <div className='w-full p-4 bg-[#6a5fdf] rounded-md text-[#d0d2d6]'>
          <div className='flex justify-center items-center py-3'>
            {
              userInfo?.image?.url ? <label htmlFor='img' className='h-[150px] w-[200px] relative p-3 cursor-pointer overflow-hidden'>
              ...
              </label> : <label className='flex justify-center items-center flex-col w-[200px] h-[150px] cursor-pointer border border-dashed hover:border-red-500 border-[#d0d2d6] relative' htmlFor='img'>
                <span><FaImages/></span>
  ...
  Remove Stripe
        ...
        <form onSubmit={handlePasswordChange}> 
                ...
                <input className='px-4 py-2 focus:border-indigo-200 outline-none bg-[#6a5fdf] border border-slate-700 rounded-md text-[#d0d2d6]' type="email" value={passwordData.email} onChange={pinputHandle} name='email' id='email' placeholder='email' />
            </div>  

            <div className='flex flex-col w-full gap-1 mb-2'>
                <label htmlFor="o_password">Old Password</label>
                <input className='px-4 py-2 focus:border-indigo-200 outline-none bg-[#6a5fdf] border border-slate-700 rounded-md text-[#d0d2d6]' type="password" name='old_password' id='o_password' value={passwordData.old_password} onChange={pinputHandle} placeholder='Old Password' />
            </div>  

            <div className='flex flex-col w-full gap-1 mb-2'>
                <label htmlFor="n_password">New Password</label>
                <input className='px-4 py-2 focus:border-indigo-200 outline-none bg-[#6a5fdf] border border-slate-700 rounded-md text-[#d0d2d6]' type="password" name='new_password' id='n_password' value={passwordData.new_password} onChange={pinputHandle} placeholder='New Password' />
            </div>   
